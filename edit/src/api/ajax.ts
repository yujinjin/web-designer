    ...
    const customerInterceptor = {
        ...
        response: {
            ...
            before: function (response: Http.Response) {
                ...
                } else if (response.data.flag === "F") {
                    if (response.config.isShowError) {
                        ElMessage.error(response.data.msg || "很抱歉，服务出错，请稍后再试~");
                    }
            ...
            error: function (error: Http.Error) {
                //请求错误时做些事
                const xhr: any = error.response && error.response.data;
                if (error.config.isShowError) {
                    ...
                    ElMessage.error(errorMesg);
                    logs.debug(errorMesg);
                }
    ...
    const customerAxiosOptions: Http.RequestConfig = {
        ...
        transformRequest: [
            function (data, headers: AxiosRequestHeaders) {
                ...
                } else if (String(headers["Content-Type"]).indexOf("multipart/form-data") != -1) {
                    return data;
                }
